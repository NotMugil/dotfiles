...
function Workspace(
  ...
  const showIcons =
    (workspace.id || showInactiveIcons) && workspace.windows.length > 0;
  return (
    <button
      onClick={() => niri.focusWorkspaceId(workspace.id)}
      className={
        workspace.is_active ? "dock-active-button" : "dock-inactive-button"
      }
    >
      {showIcons &&
        workspace.windows
          .filter((win) => win.id == workspace.active_window_id)
          .map((win: any) => (
            <icon className="dock-button-icon" icon={guessAppIcon(win)} />
          ))}
    </button>
  );
}
